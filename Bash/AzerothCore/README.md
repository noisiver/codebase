...
The table created might seem overwhelming but is actually quite simple. The id column defines the realm id, node is the node id, setting is which setting and value is the value of the setting. If id or node is set to -1 it will be applied to any realm that doesn't have a setting set with the realm and node id. All realms need every setting available even if you don't change them. The script will fail if any setting is missing. Any setting you don't want to change can be left with id and node set to -1. I recommend making a copy of each setting that you want to change and updating the id, and node if using a cluster, and use that row to change the setting. Below is a description of each setting.
