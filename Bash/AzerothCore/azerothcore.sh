...
function get_settings
{
    ...
    done <<<$(mysql --defaults-extra-file="$mysql_cnf" $mysql_database --skip-column-names -e "WITH s AS (SELECT id, setting, VALUE, ROW_NUMBER() OVER (PARTITION BY setting ORDER BY id DESC) nr FROM settings WHERE (id = $id OR id = -1)) SELECT setting, value FROM s WHERE nr = 1;" 2>&1) # WITH s AS (SELECT id, node, setting, VALUE, ROW_NUMBER() OVER (PARTITION BY setting ORDER BY id DESC) nr FROM settings WHERE (id = $id OR id = -1) AND (node = $node OR node = -1)) SELECT setting, value FROM s WHERE nr = 1
...
function get_client_files
{
    if [[ "$build_world" == "true" ]]; then
        ...
        if [[ "$world_data_version" == "0" || "$world_data_version" != "$version" || ! -d $root/source/bin/Cameras || ! -d $root/source/bin/dbc || ! -d $root/source/bin/maps || ! -d $root/source/bin/mmaps || ! -d $root/source/bin/vmaps ]]; then
            ...
            mysql --defaults-extra-file=$mysql_cnf $mysql_database -e "DELETE FROM settings WHERE id='$id' AND setting='world.data_version';INSERT INTO settings (id, setting, value, comment) VALUES ('$id', 'world.data_version', '$version', 'The installed client data version')"
            if [[ $? -ne 0 ]]; then
                notify_telegram "An error occurred while trying to download the client data files"
                rm -rf "$mysql_cnf"
                exit $?
