...
function import_database
{
    ...
    if [[ $1 == "both" ]] || [[ $1 == "auth" ]]; then
        ...
        for f in $OPTION_SOURCE_LOCATION/data/sql/updates/db_auth/*.sql; do
            ...
            if [[ ! -z `mysql --defaults-extra-file=$MYSQL_CNF --skip-column-names $OPTION_MYSQL_DATABASES_AUTH -e "SELECT * FROM updates WHERE name='$FILENAME' AND hash='${HASH@L}'"` ]]; then
                printf "${COLOR_ORANGE}Skipping "$(basename $f)"${COLOR_END}\n"
                continue;
            fi
            ...
            mysql --defaults-extra-file=$MYSQL_CNF $OPTION_MYSQL_DATABASES_AUTH -e "DELETE FROM updates WHERE name='$(basename $f)';INSERT INTO updates (name, hash, state) VALUES ('$FILENAME', '${HASH@L}', 'RELEASED')"
    ...
    if [[ $1 == "both" ]] || [[ $1 == "world" ]]; then
        ...
        for f in $OPTION_SOURCE_LOCATION/data/sql/updates/db_characters/*.sql; do
            ...
            if [[ ! -z `mysql --defaults-extra-file=$MYSQL_CNF --skip-column-names $OPTION_MYSQL_DATABASES_CHARACTERS -e "SELECT * FROM updates WHERE name='$FILENAME' AND hash='${HASH@L}'"` ]]; then
                printf "${COLOR_ORANGE}Skipping "$(basename $f)"${COLOR_END}\n"
                continue;
            fi
            ...
            # Add the hash to updates
            mysql --defaults-extra-file=$MYSQL_CNF $OPTION_MYSQL_DATABASES_CHARACTERS -e "DELETE FROM updates WHERE name='$(basename $f)';INSERT INTO updates (name, hash, state) VALUES ('$FILENAME', '${HASH@L}', 'RELEASED')"
        ...
        for f in $OPTION_SOURCE_LOCATION/data/sql/updates/db_world/*.sql; do
            ...
            # Add the hash to updates
            mysql --defaults-extra-file=$MYSQL_CNF $OPTION_MYSQL_DATABASES_WORLD -e "DELETE FROM updates WHERE name='$(basename $f)';INSERT INTO updates (name, hash, state) VALUES ('$FILENAME', '${HASH@L}', 'RELEASED')"
        ...
        if [[ $OPTION_MODULES_AHBOT_ENABLED == "true" ]]; then
            ...
            if [[ -d $OPTION_SOURCE_LOCATION/modules/mod-ah-bot/sql/world/base ]]; then
                ...
                for f in $OPTION_SOURCE_LOCATION/modules/mod-ah-bot/sql/world/base/*.sql; do
                    ...
                    # Add the hash to updates
                    mysql --defaults-extra-file=$MYSQL_CNF $OPTION_MYSQL_DATABASES_WORLD -e "DELETE FROM updates WHERE name='$(basename $f)';INSERT INTO updates (name, hash, state) VALUES ('$FILENAME', '${HASH@L}', 'CUSTOM')"
        ...
        if [[ $OPTION_MODULES_RECRUIT_A_FRIEND_ENABLED == "true" ]]; then
            ...
            if [[ -d $OPTION_SOURCE_LOCATION/modules/mod-recruitafriend/sql/auth/base ]]; then
                ...
                for f in $OPTION_SOURCE_LOCATION/modules/mod-recruitafriend/sql/auth/base/*.sql; do
                    ...
                    # Add the hash to updates
                    mysql --defaults-extra-file=$MYSQL_CNF $OPTION_MYSQL_DATABASES_AUTH -e "DELETE FROM updates WHERE name='$FILENAME';INSERT INTO updates (name, hash, state) VALUES ('$FILENAME', '${HASH@L}', 'CUSTOM')"
            ...
            if [[ -d $OPTION_SOURCE_LOCATION/modules/mod-recruitafriend/sql/characters/base ]]; then
                ...
                for f in $OPTION_SOURCE_LOCATION/modules/mod-recruitafriend/sql/characters/base/*.sql; do
                    ...
                    # Add the hash to updates
                    mysql --defaults-extra-file=$MYSQL_CNF $OPTION_MYSQL_DATABASES_CHARACTERS -e "DELETE FROM updates WHERE name='$FILENAME';INSERT INTO updates (name, hash, state) VALUES ('$FILENAME', '${HASH@L}', 'CUSTOM')"
