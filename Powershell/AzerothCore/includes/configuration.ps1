...
{
    Write-Host -ForegroundColor Green "`nLoading configuration options"
    if (-Not (Test-Path -Path $configFile))
    {
        Write-Host -ForegroundColor Yellow "Generating default configuration file"
        ...
        $XmlObjectWriter.WriteComment("Server realm zone. Set allowed alphabet in character, etc. names. 1 = development, 2 = united states")
        $XmlObjectWriter.WriteElementString("realm_zone","0")
    ...
    {
        $xml = [System.Xml.XmlDocument](Get-Content $configFile);
    }
}
