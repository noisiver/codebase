-- Config
local ENABLE_ASSISTANT_HEIRLOOMS              = true -- Enable obtaining heirlooms using the .assistant command
local ENABLE_ASSISTANT_GLYPHS                 = true -- Enable obtaining glyphs using the .assistant command
local ENABLE_ASSISTANT_GEMS                   = true -- Enable obtaining gems using the .assistant command
local ENABLE_ASSISTANT_CONTAINERS             = true -- Enable obtaining containers using the .assistant command
local ASSISTANT_CONTAINER_BAG                 = 23162 -- The container that a player obtains via the assistant
local ENABLE_ASSISTANT_UTILITIES              = true -- Enable obtaining utilities from the .assistant command. The utilities are rename, customize, faction change and race change
local ASSISTANT_UTILITIES_COST_RENAME         = 10 -- Utilities: Money required in gold to perform a name change
local ASSISTANT_UTILITIES_COST_CUSTOMIZE      = 50 -- Utilities: Money required in gold to perform a customization
local ASSISTANT_UTILITIES_COST_FACTION_CHANGE = 1000 -- Utilities: Money required in gold to perform a faction change
local ASSISTANT_UTILITIES_COST_RACE_CHANGE    = 500 -- Utilities: Money required in gold to perform a race change
local ENABLE_ASSISTANT_MISCELLANEOUS          = true -- Enable obtaining miscellaneous features from the .assistant command.
local ENABLE_ASSISTANT_MISCELLANOUS_TOTEMS    = true -- Enable obtaining shaman totems from the miscellaneous option

-- Gossip selects
local INT_HEIRLOOMS     = 100
...
function assistantOnLogin(event, player)
    if (ENABLE_ASSISTANT_HEIRLOOMS or ENABLE_ASSISTANT_GLYPHS or ENABLE_ASSISTANT_GEMS or ENABLE_ASSISTANT_CONTAINERS or ENABLE_ASSISTANT_UTILITIES or ENABLE_ASSISTANT_MISCELLANEOUS) then
...
function assistantOnGossipHello(event, player, object)
    ...
    if (ENABLE_ASSISTANT_MISCELLANEOUS and ENABLE_ASSISTANT_MISCELLANOUS_TOTEMS and player:GetClass() == CLASS_SHAMAN) then
...
function assistantOnGossipSelect(event, player, object, sender, intid, code)
    ...
    elseif (intid == INT_MISCELLANEOUS) then
        ...
        if (ENABLE_ASSISTANT_MISCELLANOUS_TOTEMS and player:GetClass() == CLASS_SHAMAN) then
